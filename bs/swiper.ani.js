...
function swiperAnimateCache() {
  for (
    allBoxes = window.document.documentElement.querySelectorAll('.ani'), i = 0;
    i < allBoxes.length;
    i++
  )
    allBoxes[i].attributes['style']
      ...
      : allBoxes[i].setAttribute('swiper-animate-style-cache', ' '), (allBoxes[
      i
    ].style.visibility =
      'hidden');
}
function swiperAnimate(a) {
  ...
  for (i = 0; i < b.length; i++)
    (b[i].style.visibility = 'visible'), (effect = b[i].attributes[
      'swiper-animate-effect'
    ]
      ? b[i].attributes['swiper-animate-effect'].value
      : ''), (b[i].className =
      b[i].className + '  ' + effect + ' ' + 'animated'), (style =
      b[i].attributes['style'].value), (duration = b[i].attributes[
...
function clearSwiperAnimate() {
  for (
    ...
    allBoxes[i].attributes['swiper-animate-style-cache'] &&
      ...
      ), (allBoxes[i].style.visibility = 'hidden'), (allBoxes[
      i
    ].className = allBoxes[i].className.replace('animated', ' ')), allBoxes[i]
      .attributes['swiper-animate-effect'] &&
      (
